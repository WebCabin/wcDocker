...
the {@link wcLayoutTable} object makes it easy to organize the contents of your {@link wcPanel}. But, sometimes you need to make some style changes to the table cells themselves! [wcLayoutTable.item]{@link wcLayoutTable#item} and [wcLayoutTable.addItem]{@link wcLayoutTable#addItem} both return you a handy [tableItem]{@link wcLayoutTable~tableItem} object which contains convenient methods for styling!

```
myPanel.addItem(domNode).css('text-align', 'right').css('border', '1px solid black').stretch('100%', '100%');
```

Here we are using the [tableItem.css]{@link wcLayoutTable~tableItem_css} and [tableItem.stretch]{@link wcLayoutTable~tableItem_stretch} functions, which can be chained!  The first provides a way to add normal style attributes to the cell while the second provides a more reliable way to set your table cell sizes.
...
Sometimes panel content needs time to load, for this {@link wcDocker} comes with a built in loading screen system.

The loading screen system comes in two forms, an individual panel loader and a full screen loader.

When an individual panel requires some loading time, you can use [wcPanel.startLoading()]{@link wcPanel#startLoading} and [wcPanel.finishLoading()]{@link wcPanel#finishLoading} to display and hide the loading screen for it.
...
Similarly, you can use [wcDocker.startLoading()]{@link wcDocker#startLoading} and [wcDocker.finishLoading()]{@link wcDocker#finishLoading} to display a loading screen for the entire view. This is especially useful as an initial loading screen for the entire page.
...
Also note that the [wcDocker.EVENT.LOADED]{@link wcDocker.EVENT} event will only be triggered after all panels have finished their individual loading screens (if they are using it).